...
def num_vals(theta=0.0, n_nodes=2):
...
def dicke_guess_controls(slh, theta, T, E0_cycles=2, nt=None):
    tgrid_start = 0
    tgrid_end = T
    n_nodes = slh.n_nodes
    if nt is None:
    ...
    mu_1 = float(get_mu_1(slh, num_vals(theta, n_nodes)))
    controls = {}
    pulse_sideband = pi_pulse(tgrid, t_start=tgrid_start, t_stop=tgrid_end,
                              mu=mu_1, cycles=E0_cycles)
    pulse_sideband.config_attribs['t_rise'] = UnitFloat(max(0.01 * T, 4.0),
                                                        'dimensionless')
    pulse_sideband.config_attribs['t_fall'] = UnitFloat(max(0.01 * T, 4.0),
                                                        'dimensionless')
...
def write_dicke_half_model(
    ...
    controls = dicke_guess_controls(slh=slh, theta=theta, T=T,
                                    E0_cycles=E0_cycles, nt=nt)
    qdyn_model = make_qdyn_oct_model(
        slh, num_vals=num_vals(theta=theta, n_nodes=n_nodes),
